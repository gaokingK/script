...
# MDL锁（metadata lock，即元数据锁）
- 元数据是一种锁机制，保证在并发请求下元数据和表结构的一致性
- 保护元数据：确保在执行DDL时，不会有其他事务对表的结构进行修改
- 协调并发操作:确保在执行DML时，不会因为表结构被修改造成数据不一致和潜在的数据损坏
- 事务T1会获取一个共享读锁，而事务T2试图获取一个排他锁（根据语句判断的）。由于共享读锁和排他锁互斥，事务T2将会被阻塞，直到事务T1提交或回滚。由于共享读锁和排他锁互斥，事务T2将会被阻塞，直到事务T1提交或回滚。
### MDL锁的类型
- MDL有5中类型
- MDL_SHARED（共享锁）：用于大多数读操作，如SELECT。允许多个事务同时获取共享锁。
- MDL_EXCLUSIVE（排他锁）：
    - 用于需要改变表结构的操作，如ALTER TABLE、DROP TABLE、RENAME TABLE。
    - 排他锁会阻止其他事务获取任何类型的MDL锁。
- MDL_SHARED_READ（共享读锁）：用于常规的读操作。允许多个事务同时读取数据，但不允许写操作。
- MDL_SHARED_WRITE（共享写锁）：用于写操作，如INSERT、UPDATE、DELETE。
    - 允许多个事务同时进行写操作，但不允许DDL操作。
- MDL_SHARED_UPGRADABLE（可升级共享锁）：
    - 初始状态是共享锁，可以升级为排他锁。
    - 适用于可能需要在操作过程中改变表结构的情况。
### 元数据锁是自动加的吗？和用lock主动添加的锁有什么区别
- MDL锁是mysql自动处理的会根据sql语句选择相应类型的锁，通常不用程序员主动调用，在事务提交或回滚时自动释放响应的锁
### 如果不使用事务，会使用到DML锁吗？
### 处理MDL锁等待
- 在大量并发情况下，DML锁等待会成为性能瓶颈，可以通过方式处理MDL锁等待问题
- 减少长事务
- 优化表结构变更
- 监控和诊断
