...
- GIL https://blog.csdn.net/weixin_36440198/article/details/113961683
    ...
    - 概念：全局解释器锁，用于同步线程的一种机制，使得任何时候仅有一个线程在执行。GIL 并不是Python的特性，只是在实现Python解析器(CPython)时引入的一个概念。换句话说，Python完全可以不依赖于GIL。Python解释器进程内的多线程是以协作多任务方式执行的，当一个线程遇到I/O操作时会释放GIL,而依赖CPU计算的线程则是执行代码量到一定的阀值，才会释放GIL。
