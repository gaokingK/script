## 本地使用docker部署showdoc 
- docker pull star7th/showdoc
- 建立目录
    ```shell
    mkdir -p /showdoc_data/html
    chmod -R 777 /showdoc_data
    ```
- 启动容器
    ```shell
    docker run -d --name showdoc --user=root --privileged=true -p 4999:80 -v /showdoc_data/html:/var/www/html/star7th/showdoc
    ```
- 打开 http://localhost:4999 访问showdoc user:showdoc password:123456

## 使用
- 单页面分享 http://90.90.67.14:4999/web/#/p/dc320d8dd0c575c2524bb36715c09716
- 项目分享 http://90.90.67.14:4999/web/#/5?page_id=24
## 遇到问题
- 按照官方文档指导步骤搭建起来的容器运行失败
    ```shell
    [root@localhost showdoc]# docker run -it star7th/showdoc /bin/sh
    standard_init_linux.go:211: exec user process caused "exec format error"
    [root@localhost showdoc]# docker run -it star7th/showdoc /bin/bash
    standard_init_linux.go:211: exec user process caused "exec format error"
    [root@localhost showdoc]# docker run -d star7th/showdoc
    ecce3172faafb8bbed19170d8461deaff7af2a4a3e7675b7c97ec51ea30bea84
    ```
    这个是因为架构的问题，此时的showdoc 的镜像只能在x86的架构上跑，不能在arm64的机器上跑。换了个x86的机器，解决。

## 开放文档接口API
- 使用开放文档接口API，可以自动化的完成很多事情
- 请求URL：`http://xx.com/server/index.php?s=/api/item/updateByApi`
- POST
- 参数:
    |参数名|必选|类型|description|
    |-|-|-|-|
    |api_key|yes|string|
    |api_token|yes|string|
    |cat_name|no|string|当页面处于目录下时，传递的目录名|
    |page_title|yes|string|
    |page_content|yes|string|页面内容，可以是markdown或者html文本|
    |s_number|no|number|页面序号|
- 失败返回
    ```json
    {
        "error_code": 10306,
        "error_message": "api_key 或 api_token 不匹配"
    }
    ```
### 同类工具、
- swagger UI
    - 在线测试API和生成API文档的工具
- mrdoc
- mindoc
    - 没有开放API
- smartwiki
    - 和mindoc相似
- dochub
    - 类似百度文库的web网站
- gitbook
    - 没有导入接口
- docmanager
    - 是个软件pass
### 增加上传文件的API
- 查出来的工具都大同小异，还是转向在showdoc上解决通过API上传图片的问题
- 通过在前台上传图片的过程中查看网络请求
    ```
    request url http://90.90.67.14:4999/server/index.php?s=/api/page/uploadImg&guid=1602312115646
    guid 1602312115646
    cookie_token 7ca3fa6cf2698d597cd70264146b0576e0b65d85a76cc8618acef05a29ed5099 
    expire_time 2021.03.29
    PHPSESSID 6fc6517a12c24b47a694528ed5585b7b
    ```
- 通过Insomnia 发送图片测试不通，转为使用curl测试， 同样无效
- 看showdoc的源码
### showdoc code
- showdoc 的后端代码结构
    ```
    Server/Application/Api/ //Application directory, basically all background apis are placed here

    Server/Application/Runtime/Logs //If there is an error log, it will print out the browser directly or print it here.

    Public/Uploads //The uploaded image is placed here

    Server/Application/Api/Lang #Backend language pack
    ```

### 测试

![aaa][csdn]

[csdn]:data:image/png;base64,