...
25. #### 绝对引用 绝对导入absolute_import和unicode_literals 与相对导入, python3 默认是绝对导入
    - from __future__ import absolute_import
    - 前者是为了解决自定义文件名和包名冲突的情况 如果自定义了一个string.py 会和库中的string冲突
    - 后者是为了解决编码问题,由于python2 是assic编码,导入此模块后,当前文件下的编码就是unicode
26. #### python传参的时候还是值引用还是地址引用[link](https://www.cnblogs.com/loleina/p/5276918.html)
...
27. ##### python 理解赋值 [link](https://blog.csdn.net/dta0502/article/details/80827359)
    - a = xxx 将对象xxx赋值给变量a
    - 赋值语句总是建立对象的引用，而不是复制对象
28. ##### b=a 是普通复制，copy是顶层复制，deepcopy是全部复制
    - 对于列表 list[:] 效果和copy一样
29. ##### 建议代码中避免出现魔数,即0, 1这样数字, 应用true/false或自定义可读性高的变量代替, 提高代码可读性可维护性
30. ##### 建议使用logging模块或者traceback来记录异常。
31. ##### 返回None无意义可以删除，函数不写return，默认返回值即为None
32. ##### eval 的灵活性和危险性
...
33. ##### (for else)/(while else)/(try else)
...
39. ##### 字符串连接时编译和运行
    ...
    ```
40. ##### [Python 中 is 和 == 的不同]https://blog.csdn.net/qq_34152244/article/details/100639985)
    ...
    - ###### 类的实例即使值一样，== 也是返回false
    - ###### 注意小整数池和字符串池的影响
41. ##### python 解释器中的小整数池机制和关于字符串的intern 机制（字符串驻留）
...
43. ##### step out、step into、step into mycode run to cursor 
    对语法糖没有用
    step out  是跳出当前执行的子函数 
    step into 是进入子函数（如print就不会进入，yaml.load_all()就会进入）
    step into mycode 是进入自己子函数中
44. ##### [UnicodeDecodeError 这个是编码问题](https://blog.csdn.net/qq284489030/article/details/80561963)
...
45. ##### [CPython、IPython等不同的python解释器](https://blog.csdn.net/xyisv/article/details/79389626)
...
46. ##### [Java中字节码和机器指令、JIT技术](https://blog.csdn.net/TheLudlows/article/details/87568456)
    - 字节码不可以被机器直接运行，而是经过JVM翻译成对应的机器指令，逐条读入，逐条翻译解释才能运行
47. ##### python 只有6种数据类型，里面没有float，python中是怎么保存浮点数呢
...
49. ##### `python -m venv` -m参数的含义 相当于import,叫做当做模块来启动，不同的加载py文件的方式，主要是影响——sys.path 这个属性。sys.path 就相当于liunx中的PATH。
50. ##### 使用python3的venv来创建虚拟环境
    ```python
