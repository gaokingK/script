...
if __name__ == "__main__":
    ...
    with open(os.path.join(os.path.dirname(__file__), "bizsser_all.json"), "r", encoding='utf-8') as f:
    # all_data = json.load("./Aiops/biszer.json", data.decode('utf-8')) 
        all_data = json.load(f)
    res_file = os.path.join(os.path.dirname(__file__), "应用监控指标AI纳管明细.xlsm")
    ...
    for app in all_data:
        ...
        rules_list = []
        formate_data([app])
        if app["name"] in  wb.sheetnames:
            wb.remove(wb[app["name"]])
        ...
        work_sheet.column_dimensions['D'].width=120
        for i in range(1,5):
