...
## base64编码
Base64 编码是将每 6 位二进制数据编码为一个字符，从而可以将二进制数据转换为只包含 ASCII 字符的文本格式。
Base64 编码使用一个特定的字符集来表示 64 个不同的值。这些字符包括：
大写字母：A-Z
小写字母：a-z
数字：0-9
加号 + 和斜杠 /
如果结果不是4倍的整数，会用=来补齐base64编码是把3个字节转换成4个可打印字符，所以编码长度一定可以被4整除，如果原文不能被3整除，那么结果就会出现=去补齐
需要注意的是，/ 字符在某些情况下（如 URL 或文件名中）可能会造成问题。为了避免这些问题，Base64 编码有一个变种称为 Base64 URL 编码，其中 + 和 / 被替换为   和 _。这种变种可以确保编码结果在 URL 和文件名中是安全的：
"""
